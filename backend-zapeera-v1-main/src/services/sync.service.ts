...
class SyncService {
  ...
  async checkDatabaseHealth(): Promise<{
    ...
    try {
      ...
      const userCount = await sqliteClient.zapeeraUser.count();
  ...
  async syncUsersFromPostgreSQL(): Promise<{ synced: number; errors: string[] }> {
    ...
    try {
      ...
      const columnCheck = await pgClient.query(`
        ...
        WHERE table_name = 'zapeera_users'
      `);
      const existingColumns = columnCheck.rows.map((r: any) => r.column_name);

      // Build query with only existing columns
      const baseColumns = ['id', 'email', 'password', 'name', 'role', 'isActive', 'businessAccessGranted', 'createdAt', 'updatedAt'];
      ...
      const pgUsers = await pgClient.query(`SELECT ${selectClause} FROM zapeera_users`);
      ...
      for (const pgUser of pgUsers.rows) {
        try {
          ...
          let existingUser = await sqliteClient.zapeeraUser.findUnique({
            where: { id: pgUser.id }
          });

          // Also check by email (in case ID is different but same email)
          if (!existingUser && pgUser.email) {
            existingUser = await sqliteClient.zapeeraUser.findUnique({
          ...
          const updateData: any = {
            ...
            name: pgUser.name,
            isActive: isActive,
            businessAccessGranted: pgUser.businessAccessGranted === undefined ? true : Boolean(pgUser.businessAccessGranted),
            updatedAt: new Date()
          };
          ...
          if (pgUser.companyId) {
            try {
              const companyExists = await sqliteClient.business.findUnique({ where: { id: pgUser.companyId } });
          ...
          // CRITICAL FIX: sessionToken handling is now done in the existingUser block below
          // This ensures proper preservation logic
          ...
          if (existingUser) {
            // CRITICAL: ALWAYS preserve sessionToken if it exists in SQLite
            // Don't overwrite active sessions during sync - this prevents logout
            // sessionToken should ONLY be updated on login, NEVER during sync
            if (existingUser.sessionToken) {
              updateData.sessionToken = existingUser.sessionToken;
              console.log(`[Sync] 🔒 Preserving sessionToken for user: ${pgUser.email} (preventing logout)`);
            } else {
              // Only set sessionToken from PostgreSQL if SQLite doesn't have one AND PostgreSQL has one
              // This handles first-time sync scenarios
              if (pgUser.sessionToken) {
                updateData.sessionToken = pgUser.sessionToken;
                console.log(`[Sync] 🔑 Setting sessionToken from PostgreSQL for user: ${pgUser.email} (first-time sync)`);
              }
            }

            // Update existing user by their actual ID
            await sqliteClient.zapeeraUser.update({
              where: { id: existingUser.id },
              data: updateData
            });
            console.log(`[Sync] ✅ Updated user: ${pgUser.email} (isActive: ${isActive}, sessionToken preserved: ${!!existingUser.sessionToken})`);
          } else {
            // Create new user - need to ensure username is unique
            let username = pgUser.username || pgUser.email;

            // Check if username already exists
            const usernameExists = await sqliteClient.zapeeraUser.findFirst({
            ...
            await sqliteClient.zapeeraUser.create({
              data: createData
            });
  ...
  async syncAllTablesFromPostgreSQL(): Promise<{ synced: number; failed: number; errors: string[] }> {
    ...
    try {
      ...
      const tablesToSync = [
        ...
        { pg: 'modules', sqlite: 'module' },
        { pg: 'business_types', sqlite: 'businessType' },
        { pg: 'business_type_modules', sqlite: 'businessTypeModule' },
  ...
  async syncAllTablesFromSQLiteToPostgreSQL(): Promise<{
    ...
    const tables = [
      ...
      // Users ARE synced TO PostgreSQL (so local users can be pushed to cloud)
  ...
  async pullTableFromPostgreSQL(tableName: string): Promise<{ synced: number; failed: number }> {
    ...
    // Users created locally should NEVER be overwritten by PostgreSQL data
    // This prevents staff from disappearing when net is turned on
    if (tableName === 'user') {
      console.log(`[Sync] ⚠️ SKIPPING user pull - users should only sync TO PostgreSQL, not FROM PostgreSQL`);
      return result;
    }
    ...
    try {
      ...
      for (const record of records) {
        try {
          ...
          if (existingRecord) {
            // CRITICAL FIX: For users table, NEVER overwrite local SQLite users with PostgreSQL data
            // Local users take absolute precedence - they should only sync TO PostgreSQL, not FROM
            if (tableName === 'user') {
              console.log(`[Sync] ⚠️ Skipping user ${recordId} - local SQLite users should not be overwritten by PostgreSQL`);
              continue; // Skip this user - don't update or delete
            }
  ...
  async syncBidirectionalImmediate(
    ...
    // CRITICAL FIX: Don't pull users - they should only sync TO PostgreSQL, not FROM
    // This prevents staff from disappearing when net is turned on
    let pulled = { synced: 0, failed: 0 };
    if (tableName !== 'user') {
      pulled = await this.pullTableFromPostgreSQL(tableName);
    } else {
      console.log(`[Sync] ⚠️ Skipping bidirectional pull for users - users should only sync TO PostgreSQL`);
    }
