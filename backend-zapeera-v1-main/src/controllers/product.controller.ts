...
export const getProducts = async (req: AuthRequest, res: Response) => {
  try {
    ...
    console.log('[Products] getProducts where clause:', JSON.stringify(where));
    ...
    console.log(`[Products] Fetched ${products.length} products (total=${total}) for user ${req.user?.id}, role=${req.membership?.role_name || req.user?.role}`);
    if (products.length > 0) {
      const firstProductStock = products[0].batches.reduce((sum, batch) => sum + batch.quantity, 0);
      console.log(`[Products] First product: branchId=${products[0].branchId}, calculatedStock=${firstProductStock}, minStock=${products[0].minStock}, name=${products[0].name}`);
    }
    ...
    const productsWithBatchData = products.map(product => {
      ...
      return {
        ...
        unitsPerPack: product.unitsPerPack && product.unitsPerPack > 1
          ? product.unitsPerPack
          : (currentBatch?.unitsPerBox || 1)
      };
    });
...
export const createProduct = async (req: AuthRequest, res: Response) => {
  try {
    ...
    // Use resolved company/branch context if available, otherwise use the provided branchId
    let targetCompanyId: string;
    let targetBranchId: string;

    const resolvedCompanyId =
      req.business_id || headerCompanyId || req.user?.selectedCompanyId;
    const resolvedBranchId =
      req.branch_id || headerBranchId || req.user?.selectedBranchId;

    if (resolvedCompanyId && resolvedBranchId) {
      // Use selected company/branch context
      targetCompanyId = String(resolvedCompanyId);
      targetBranchId = String(resolvedBranchId);
...
export const updateProduct = async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (updateData.formula !== undefined) updateFields.formula = updateData.formula || null; // CRITICAL: Include formula field
    ...
    if (updateData.supplierId !== undefined) updateFields.supplierId = updateData.supplierId || null;
    if (updateData.branchId !== undefined) updateFields.branchId = updateData.branchId;
    if (updateData.minStock !== undefined) updateFields.minStock = updateData.minStock;
    ...
    console.log('🔍 Updating product with fields:', Object.keys(updateFields));
      ...
      try {
        ...
        if (!productData.branchId) {
          ...
          if (availableBranch) {
            ...
            const defaultCompany = await prisma.business.create({
              data: {
...
export const getStockMovements = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    if (branchIdStr && allowedBranchIds.includes(branchIdStr)) {
      where.product = {
        branchId: branchIdStr
      };
    } else if (allowedBranchIds.length > 0) {
      // For users with branch access, ensure we only get stock movements for products in their allowed branches
      where.product = {
