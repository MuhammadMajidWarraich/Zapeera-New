...
const TABLE_MAPPINGS: Record<string, string> = {
  ...
  'membershipBranch': 'membership_branches',
...
export async function syncAfterOperation(
  ...
  // CRITICAL FIX: Don't block operations if PostgreSQL connection fails
  // In Electron mode, SQLite is primary - sync is optional
  try {
    ...
    try {
      ...
      // CRITICAL: Skip pull for branch updates to prevent user sessionToken issues
      // Branch updates should only push to PostgreSQL, not pull (to avoid overwriting user data)
      if (tableName !== 'branch') {
        ...
        console.log(`[Sync-Helper] ⚠️ Skipping pull for branch update - prevents user sessionToken conflicts`);
      }

      return pushSuccess;
    } catch (error: any) {
      console.error(`[Sync-Helper] ❌ Sync failed: ${error.message}`);
      // CRITICAL: Don't throw - just queue for retry and return false
      // This ensures the main operation (create/update/delete) succeeds even if sync fails
      syncService.addToQueue(tableName, operation, record);
      return false;
    }
  } catch (error: any) {
    // CRITICAL FIX: Catch any errors (like PostgreSQL connection failures) and don't block
    console.error(`[Sync-Helper] ❌ Fatal sync error (non-blocking): ${error.message}`);
    // Return false but don't throw - main operation should succeed
    return false;
  }
}
