...
/**
 * Build a business-scoped Prisma filter for models that are related through memberships.
 * This ensures queries only return records belonging to the requested business.
 */
export function withBusinessScope(
  businessId: string,
  options?: { status?: string }
): any {
  if (!businessId || typeof businessId !== 'string' || businessId.trim() === '') {
    throw new Error('withBusinessScope requires a valid businessId');
  }
  ...
  if (options?.status) {
    membershipFilter.status = options.status;
  }
